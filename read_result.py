...
if not result_files:
    ...
    #Пробразование дат из Unix в московское время
    start_data = data['start']
    end_data = data['stop']
    #Начало текста в московское время
    utc_time_start = pendulum.from_timestamp(start_data / 1000, tz='UTC')
    moscow_time_start = utc_time_start.in_tz('Europe/Moscow')
    #Окончание
    utc_time_end = pendulum.from_timestamp(start_data / 1000, tz='UTC')
    moscow_time_end = utc_time_end .in_tz('Europe/Moscow')
    ...
    for step in data.get('steps', []):
        ...
        utc_time_end = pendulum.from_timestamp(start_data / 1000, tz='UTC')
        moscow_time_end = utc_time_end.in_tz('Europe/Moscow')
        result_text += f"- {step['name']}: {step['status']} (начало: {moscow_time_start}, конец: {moscow_time_end})\n"
